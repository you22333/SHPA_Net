...
from news.FFCM import Fused_Fourier_Conv_Mixer
from news.MAIR import ShuffleAttn
from news.MEEM import MEEM
from news.SSFF import SSFF
from news.FCA import FCAttention
from news.lka import LKA
from news.CBAM import CBAM
from news.MSTF import MSTF
from news.gate import gatedFusion
from news.SENet import SEAttention
from news.ffcm import FFCM
...
class FewShotSeg(nn.Module):
    def __init__(self, pretrained_weights="deeplabv3"):
        ...
        self.ffcm = Fused_Fourier_Conv_Mixer(512)
        self.mai =ShuffleAttn(512, 512)
        self.meem = MEEM(512, 512)
        self.ssff = SSFF(inc=[64, 128, 256], channel=512)
        self.fca = FCAttention(channel=512)
        self.lka = LKA(512)
        self.cba = CBAM(512)
        self.gate = gatedFusion(dim= 65)
        self.se = SEAttention(channel=512,reduction=8)
        self.ffcm = FFCM(dim= 512)
        self.sa = StripeAttentionBlock(d_model=512, k1=1, k2=19)
        self.agnn = HyperNet(channel=512, node=65, kernel_size=3, stride=1, K_neigs=[3])
    ...
    def forward(self, supp_imgs, supp_mask, qry_imgs, train=False):
        ...
        supp_fts = 0.9 * supp_fts + 0.1 * qry_fts
