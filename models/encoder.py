...
class Res101Encoder(nn.Module):
    ...
    def __init__(self, replace_stride_with_dilation=None, pretrained_weights='resnet101'):
        ...
        if pretrained_weights == 'deeplabv3':
            self.pretrained_weights = torch.load(
                "/home/admin111/UU-base/RPT-main/deeplabv3_resnet101_coco-586e9e4e.pth", map_location='cpu')
        ...
        self._init_weights()
        self.dwonsample = nn.MaxPool2d(kernel_size=8, stride=8)
        ...
        self.agnn = HyperNet(channel=512, node=8, kernel_size=3, stride=1, K_neigs=[3])
    ...
    def forward(self, x):
        ...
        # x1 = self.dwonsample(x)
        # x1 = self.agnn(x1)
        # x1 =self.align_channels(x1)

        x = self.backbone["layer2"](x)
        # x2 = self.dwonsample(x)
        x = self.backbone["layer3"](x)
        # x3 = self.dwonsample(x)
